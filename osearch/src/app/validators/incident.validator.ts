...
export class IncidentValidators {
  ...
  static validateZipcode(zcs: ZipCode[]): ValidatorFn {
    return (c: AbstractControl): {[key: string]: any}| null => {
      if (c.value && c.value !== undefined) {
        const zips = zcs.filter(zc => zc.zipCode1 === c.value);
        if (zips.length === 0 ) {
          // c.setErrors({'validOregonZipRequired': true});
          return {validOregonZipRequired: true};
        }
      }
    };
  }
